  ...
  const renderItemContent = (item) => {
    if (item.original_title && item.media_type === 'movie') {
      return (
        ...
        >
          <Text style={tw`w-full font-medium text-lg ${text}`}>{`${
            item.original_title
      ...
      if (item.media_type === 'person') {
        return (
          ...
          >
            <Text style={tw`w-full font-medium text-lg ${text}`}>{`${item.name} / ${t(
              'utils.celebrity'
            )}`}</Text>
          </TouchableOpacity>
        )
        ...
        <View style={tw`flex-1 justify-start items-stretch ${background}`}>
          ...
          <View style={tw`px-4 mt-4 relative`}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                return (
                  <View style={tw`ml-auto mr-auto flex-wrap w-full ${background}`}>
